...
class BaseRBM(EnergyBasedModel):
    ...
    def _make_vars(self):
        ...
        if self.display_filters:
            with tf.compat.v1.name_scope('filters_visualization'):
                W_display = tf.transpose(a=self._W, perm=[1, 0])
    ...
    def _sample_h_given_v(self, h_means):
        """Sample from P(h|v)."""
        with tf.compat.v1.name_scope('sample_h_given_v'):
            h_samples = self._h_layer.sample(means=h_means)
        return h_samples

    def _means_v_given_h(self, h):
        """Compute means E(v|h)."""
        with tf.compat.v1.name_scope('means_v_given_h'):
    ...
    def _sample_v_given_h(self, v_means):
        """Sample from P(v|h)."""
        with tf.compat.v1.name_scope('sample_v_given_h'):
    ...
    def transform_op(self, x_batch):
        if self.dropout is not None:
            x_batch = tf.nn.dropout(x_batch, rate=1 - (self._dropout))
    ...
    def train_op(self, x_batch, run_metrics=False):
        ...
        if self.display_hidden_activations:
            ...
            h_means_display = tf.expand_dims(h_means_display, 0)
            h_means_display = tf.expand_dims(h_means_display, -1)
            # tf.compat.v1.summary.image('hidden_activation_means', h_means_display)
            tf.summary.image('hidden_activation_means', h_means_display)
        ...
        with tf.compat.v1.name_scope('grads_estimates'):
            ...
            N = tf.cast(tf.shape(input=x_batch)[0], dtype=self._tf_dtype)
            with tf.compat.v1.name_scope('dW'):
                dW_positive = tf.matmul(x_batch, h0_means, transpose_a=True)
            ...
            with tf.compat.v1.name_scope('dvb'):
                dvb = tf.reduce_mean(input_tensor=x_batch - v_states, axis=0)  # == sum / N
            with tf.compat.v1.name_scope('dhb'):
                dhb = tf.reduce_mean(input_tensor=h0_means - h_means, axis=0)  # == sum / N

        # apply sparsity targets if needed
        with tf.compat.v1.name_scope('sparsity_targets'):
            q_means = tf.reduce_sum(input_tensor=h_means, axis=0)
        ...
        # update parameters
        self._learning_rate = self.get_current_learning_rate()
        self._momentum = self.get_current_momentum()
        ...
        if run_metrics:
            ...
            with tf.compat.v1.name_scope('mean_squared_recon_error'):
            ...
            with tf.compat.v1.name_scope('pseudo_loglik'):
                x = x_batch
            ...
            free_energy_op = self._free_energy(x_batch)
            tf.compat.v1.add_to_collection('free_energy_op', free_energy_op)

            # collect summaries
            if self.metrics_config['l2_loss']:
                tf.compat.v1.summary.scalar(self._metrics_names_map['l2_loss'], l2_loss)
            if self.metrics_config['msre']:
                tf.compat.v1.summary.scalar(self._metrics_names_map['msre'], msre)
            if self.metrics_config['pll']:
                tf.compat.v1.summary.scalar(self._metrics_names_map['pll'], pll)
            return (W_update, vb_update, hb_update), (l2_loss, msre, pll)

        return (W_update, vb_update, hb_update), (None, None, None)
    ...
    def _train_epoch(self, X):
        results = [[] for _ in range(len(self._train_metrics_names))]
        for X_batch in batch_iter(X, self.batch_size,
                                  verbose=self.verbose):
            self.iter_ += 1
            if self.iter_ % self.metrics_config['train_metrics_every_iter'] == 0:
                (W_update, vb_update, hb_update), (l2_loss, msre, pll) = self.train_op(X_batch, run_metrics=True)
                for i, v in enumerate((l2_loss, msre, pll)):
                    results[i].append(v)
            else:
                (W_update, vb_update, hb_update), (_, _, _) = self.train_op(X_batch)
        ...
        return dict(zip(self._train_metrics_names, results))



    def eval_metrics(self, x_batch):
    ...
    def _run_val_metrics(self, X_val):
        results = [[] for _ in range(len(self._val_metrics_names))]
        for X_vb in batch_iter(X_val, batch_size=self.batch_size):

            msre, pll = self.eval_metrics(X_vb)

            for i, v in enumerate((msre, pll)):
                results[i].append(v)
        ...
        return dict(zip(self._val_metrics_names, results))
    ...
    def _run_feg(self, X, X_val):
        ...
        for _, X_b in zip(range(self.metrics_config['n_batches_for_feg']),
            ...
            train_fe = self._free_energy(X_b)
    ...
    def transform(self, X, np_dtype=None):
        ...
        for X_b in batch_iter(X, batch_size=self.batch_size,
            ...
            H_b = self.transform_op(X_b)
            # H_b = self._transform_op.eval(feed_dict=self._make_tf_feed_dict(X_b), session=tf.compat.v1.get_default_session())
