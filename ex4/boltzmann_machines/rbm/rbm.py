...
class BernoulliRBM(BaseRBM):
    ...
    def _free_energy(self, v):
        with tf.compat.v1.name_scope('free_energy'):
            ...
            T2 = -tf.reduce_sum(input_tensor=tf.nn.softplus(self._propup(v) + self._hb), axis=1)
            fe = tf.reduce_mean(input_tensor=T1 + T2, axis=0)
        return fe
...
class MultinomialRBM(BaseRBM):
    ...
    def _free_energy(self, v):
        ...
        with tf.compat.v1.name_scope('free_energy'):
            ...
            fe = tf.reduce_mean(input_tensor=T1 + T3, axis=0)
            fe += -tf.math.lgamma(M + K) + tf.math.lgamma(M + 1) + tf.math.lgamma(K)
        return fe
...
class GaussianRBM(BaseRBM):
    ...
    def _free_energy(self, v):
        with tf.compat.v1.name_scope('free_energy'):
            ...
            T3 = 0.5 * tf.reduce_sum(input_tensor=T2, axis=1)
            T4 = -tf.reduce_sum(input_tensor=tf.nn.softplus(self._propup(v) + self._hb), axis=1)
            fe = tf.reduce_mean(input_tensor=T3 + T4, axis=0)
        return fe
